...
#include <iomanip>
#ifdef _WIN32
// Use Windows CryptoAPI instead of OpenSSL on Windows
#include <windows.h>
#include <wincrypt.h>
#else
...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false), mpPathRecorder(nullptr), mpPathGuide(nullptr)
{
    ...
    //Initialize the Path Recorder and Path Guide
    ...
    //if(false) // TODO
    {
        ...
        #ifdef __APPLE__
        // On macOS, don't create viewer thread automatically - let main thread handle it
        mptViewer = nullptr;
        #else
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // Add pose to path recording if active
    if (mpPathRecorder && mpPathRecorder->IsRecording() && !Tcw.translation().hasNaN()) {
        // Convert camera pose to world pose (invert Tcw)
        Sophus::SE3f Twc = Tcw.inverse();
        float confidence = (mTrackingState == 2) ? 1.0f : 0.5f; // Full confidence for OK tracking
        mpPathRecorder->AddPose(timestamp, Twc, mTrackingState, confidence);
    }
    
    // Update path guidance if active
    if (mpPathGuide && mpPathGuide->IsGuidanceActive() && !Tcw.translation().hasNaN()) {
        Sophus::SE3f Twc = Tcw.inverse();
        mpPathGuide->UpdateCurrentPose(Twc);
    }
...
void System::Shutdown()
{
    ...
    // Stop path recording if active
    if(mpPathRecorder) {
        mpPathRecorder->StopRecording();
        delete mpPathRecorder;
        mpPathRecorder = nullptr;
    }
...
void System::StartPathRecording(const std::string& output_filename)
{
    if (mpPathRecorder) {
        mpPathRecorder->StartRecording(output_filename);
        cout << "System: Path recording started - " << output_filename << endl;
    }
}
...
void System::StopPathRecording()
{
    if (mpPathRecorder) {
        mpPathRecorder->StopRecording();
        cout << "System: Path recording stopped" << endl;
    }
}

bool System::IsPathRecording() const
{
    return mpPathRecorder ? mpPathRecorder->IsRecording() : false;
}

size_t System::GetRecordedPointsCount() const
{
    return mpPathRecorder ? mpPathRecorder->GetRecordedPointsCount() : 0;
}

double System::GetPathRecordingDuration() const
{
    return mpPathRecorder ? mpPathRecorder->GetRecordingDuration() : 0.0;
}

// Path Guidance functionality
bool System::LoadPathForGuidance(const std::string& path_filename)
{
    if (mpPathGuide) {
        return mpPathGuide->LoadPath(path_filename);
    }
    return false;
}

bool System::StartPathGuidance()
{
    if (mpPathGuide) {
        return mpPathGuide->StartGuidance();
    }
    return false;
}

void System::StopPathGuidance()
{
    if (mpPathGuide) {
        mpPathGuide->StopGuidance();
    }
}

bool System::IsGuidanceActive() const
{
    if (mpPathGuide) {
        return (mpPathGuide->GetState() == GuidanceState::GUIDING);
    }
    return false;
}

std::string System::GetGuidanceStatus() const
{
    if (mpPathGuide) {
        return mpPathGuide->GetStatusString();
    }
    return "No guidance system";
}

float System::GetGuidanceProgress() const
{
    if (mpPathGuide) {
        return mpPathGuide->GetPathProgress();
    }
    return 0.0f;
}

void System::SetBackwardsNavigationMode(bool backwards)
{
    if (mpPathGuide) {
        mpPathGuide->SetBackwardsMode(backwards);
    }
}

bool System::IsBackwardsNavigationMode() const
{
    if (mpPathGuide) {
        return mpPathGuide->IsBackwardsMode();
    }
    return false;
}
...
AudioGuide* System::GetAudioGuide()
{
    if (mpPathGuide) {
        return mpPathGuide->GetAudioGuide();
    }
    return nullptr;
}

void System::SpeakAudioMessage(const std::string& message, int priority)
{
    if (mpPathGuide) {
        AudioGuide* audioGuide = mpPathGuide->GetAudioGuide();
        if (audioGuide) {
            audioGuide->Speak(message, static_cast<AudioPriority>(priority));
        }
    }
}
...
string System::CalculateCheckSum(string filename, int type)
{
    ...
    {
        cout << "[E] Unable to acquire crypt context for MD5 hash." << endl;
        f.close();
        return checksum;
    }
    ...
    {
        cout << "[E] Unable to create MD5 hash." << endl;
    ...
    {
        if (!CryptHashData(hHash, (BYTE*)buffer, count, 0))
        {
            cout << "[E] Unable to hash data." << endl;
    ...
    if (CryptGetHashParam(hHash, HP_HASHVAL, c, &cbHash, 0))
    {
        // Success - hash is in c[]
    }
    else
    {
        cout << "[E] Unable to get hash value." << endl;
    }
...
#ifdef __APPLE__
void System::RunViewerOnMainThread()
{
    if(mpViewer)
    {
        try {
            ...
            std::cerr << "Exception in viewer: " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "Unknown exception in viewer" << std::endl;
        }
    }
}

bool System::IsViewerRunning()
{
    if(mpViewer)
    {
        try {
