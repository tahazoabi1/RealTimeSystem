...
#ifdef _WIN32
// Windows doesn't have sys/wait.h, sys/prctl.h, or the same signal.h
// These are typically used for process management which may not be needed
#include <csignal>
#else
#include <sys/wait.h>    
#include <signal.h>     
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    ////// WALL ----------------------- DETECTOR--------------------
   int featureCount = mCurrentFrame.mvKeys.size();
    
    // Estimate wall distance using existing map points if available
    float estimated_wall_distance = 2.0f;  // Default 2 meters
    if (mState == OK && !mCurrentFrame.mvpMapPoints.empty()) {
        std::vector<float> depths;
	Eigen::Vector3f camera_center = mCurrentFrame.GetCameraCenter();
        ...
        for (size_t i = 0; i < mCurrentFrame.mvpMapPoints.size(); ++i) {
            ...
            if (pMP && !pMP->isBad()) {
                ...
                Eigen::Vector3f diff = pos - camera_center;
                float depth = diff.norm();  
                if (depth > 0.1f && depth < 10.0f) {  // Reasonable depth range
        ...
        if (!depths.empty()) {
            ...
            estimated_wall_distance = depths[depths.size()/2];
        }
    }
    
    // Process wall detection
    bool wall_detected = mWallDetector.processFrame(featureCount, mImGray, estimated_wall_distance);
    
    // Handle wall detection
    if (wall_detected) {
        int consecutive_frames = mWallDetector.getConsecutiveFrames();
        std::cout << "[WallDetector] Frame " << mCurrentFrame.mnId 
                  << " sees wall (" << featureCount << " features, " 
                  << consecutive_frames << " consecutive frames)" << std::endl;
        
        // Strategy: Graceful degradation instead of immediate failure
        if (consecutive_frames > 5 && mState == OK) {
            // Don't immediately set to LOST, use a transitional state
            std::cout << "[Tracking] Entering wall-detection mode - reduced tracking" << std::endl;
            
            // Option 1: Disable mapping temporarily but continue tracking
            mbOnlyTracking = true;
            
            // Option 2: Use constant velocity model if we have previous poses
            if (!mLastFrame.GetPose().matrix().isZero()) {
                // Predict pose based on previous motion
                Sophus::SE3f last_pose = mLastFrame.GetPose();
                if (!mPrevFrame.GetPose().matrix().isZero()) {
                    // Calculate velocity between last two frames
                    Sophus::SE3f velocity = mPrevFrame.GetPose().inverse() * last_pose;
                    Sophus::SE3f predicted_pose = last_pose * velocity;
                    mCurrentFrame.SetPose(predicted_pose);
                } else {
                    // No velocity info, use last known pose
                    mCurrentFrame.SetPose(last_pose);
                }
            }
        }
    } else if (mWallDetector.getConsecutiveFrames() == 0 && mbOnlyTracking) {
        // Wall detection ended, resume normal operation
        std::cout << "[Tracking] Wall cleared - resuming normal SLAM" << std::endl;
        mbOnlyTracking = false;
    }
    
    // Write enhanced camera position with orientation (for map visualizer)
    //writeCameraPositionWithOrientation();
...
void Tracking::writeCameraPositionWithOrientation() {
    if (mState == OK && !mCurrentFrame.GetPose().matrix().isZero()) {
        // Get camera pose (SE3)
        Sophus::SE3f pose = mCurrentFrame.GetPose();

        // Extract translation (camera position in world coordinates)
        Eigen::Vector3f twc = pose.translation(); // CORRECT: directly use translation

        // Extract rotation matrix and convert to Euler angles
        Eigen::Matrix3f Rwc = pose.rotationMatrix().transpose();

        // Convert rotation matrix to Euler angles (ZYX convention)
        float roll = atan2(Rwc(2,1), Rwc(2,2));
        float pitch = atan2(-Rwc(2,0), sqrt(Rwc(2,1)*Rwc(2,1) + Rwc(2,2)*Rwc(2,2)));
        float yaw = atan2(Rwc(1,0), Rwc(0,0));

        // Write to file
        std::ofstream cam_file("CameraPosition.txt", std::ios::trunc);
        if (cam_file.is_open()) {
            cam_file << std::fixed << std::setprecision(6)
                     << twc(0) << " " << twc(1) << " " << twc(2) << " "
